...
public:
     ...
     * @return 1 when the operation has been successful, 0 when it fails
     */
    int add(const std::string& line); //H
     ...
     */
    const Event & at(int pos) const; //H
 ...
 */
float sumPrice(const EventSet & evSet); //H
 ...
 */
void findUnique(const EventSet& es, const std::string &field, std::string values[], int & n_values); //H
 ...
 * @deprecated
 */
EventSet rawFilterEvents(const EventSet& es, const std::string &field, const std::string &value); //H
