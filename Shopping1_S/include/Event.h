/**
 * @file Event.h
 * @author MP-Team DECSAI && Manuel Fuentes Plaza && Noelia Carrasco Vilar
 * @date 22/03/2021
 */
...
const string EVENT_DEFAULT = (((string)"1971-01-01 00:00:00 UTC,view,") + EMPTY_FIELD + ",,,,-1.000000," + EMPTY_FIELD + "," + EMPTY_FIELD);
/// The set  of valid types. The default time is the first one
const string VALID_TYPES[] = {"view", "cart", "purchase", "remove_from_cart"};
...
private:
    DateTime _dateTime;
    string _type;
    string _prod_id; // NOTNULL
    string _cat_id;
    string _cat_cod;
    string _brand;
    double _price;
    string _user_id; //NOTNULL
    string _session; //NOTNULL
    void split(string l, string date, string time);
    void initDefault();
